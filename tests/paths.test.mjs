...
import { inspectPaths, parseManifest, portableKey, LIMITS, manifestJSON } from '../core/paths.mjs';

const targets = (paths, options) => inspectPaths(paths, options).entries.map((entry) => entry.target);
...
test('ordinary trees keep their names, extensions and input order', () => {
  ...
  assert.deepEqual(report.entries.map((entry) => entry.target), input);
...
test('reserved Windows device names include extensions and superscript digits', () => {
  assert.deepEqual(targets(['CON.pdf', 'com¹.txt', 'LPT²', 'aux', 'COM0.txt', 'COM10.txt']), ['_CON.pdf', '_com¹.txt', '_LPT²', '_aux', 'COM0.txt', 'COM10.txt']);
});

test('literal backslashes, invalid characters, trailing spaces and bidi controls are repaired', () => {
  assert.deepEqual(targets(['my\\file?.txt', 'report. ', 'invisible\u202eexe.txt', 'emoji🙂.txt']), ['my_file_.txt', 'report', 'invisible_exe.txt', 'emoji🙂.txt']);
});

test('directory case collisions do not merge their children', () => {
  const report = inspectPaths(['Docs/one.txt', 'docs/two.txt', 'docs/three.txt']);
  assert.deepEqual(report.entries.map((entry) => entry.target), ['Docs/one.txt', 'docs~2/two.txt', 'docs~2/three.txt']);
...
test('Unicode normalization conflicts are reported and resolved', () => {
  ...
  assert.deepEqual(report.entries.map((entry) => entry.target), ['photos/café.jpg', 'photos/café~2.jpg']);
  assert.equal(report.counts['unicode-normalization'], 1);
});

test('allocation preserves an existing suffixed name', () => {
  assert.deepEqual(targets(['Logo.svg', 'logo.svg', 'logo~2.svg']), ['Logo.svg', 'logo~3.svg', 'logo~2.svg']);
});

test('repair collisions and prototype-shaped names cannot overwrite files', () => {
  assert.deepEqual(targets(['CON.txt', '_CON.txt', '__proto__', 'constructor', 'a?.txt', 'a*.txt']), ['_CON~2.txt', '_CON.txt', '__proto__', 'constructor', 'a_~2.txt', 'a_.txt']);
});

test('a file and a same-named directory get separate destination names', () => {
  const report = inspectPaths(['folder', 'folder/child.txt']);
  assert.deepEqual(report.entries.map((entry) => entry.target), ['folder~2', 'folder/child.txt']);
...
test('absolute paths and traversal segments never reach a portable plan', () => {
  for (const path of ['/a', '\\server\\share', 'C:/a', '../a', 'a/../b', './a', 'a//b', 'a/']) assert.throws(() => inspectPaths([path]), /absolute|segments/);
});
...
test('destination root and the files wrapper count toward the path budget', () => {
  ...
  for (const rootLength of [-1, 201, NaN, 1.5, '40']) assert.throws(() => inspectPaths(['a'], { rootLength }), /root length/);
});

test('work is bounded by files, tree depth, nodes, path length and input size', () => {
  assert.throws(() => inspectPaths(Array(LIMITS.files + 1).fill('a')), /between/);
  assert.throws(() => inspectPaths(['a/'.repeat(32) + 'file']), /levels/);
  assert.throws(() => inspectPaths(['a'.repeat(4097)]), /4096/);
  assert.throws(() => parseManifest('a'.repeat(LIMITS.inputChars + 1)), /2 million/);
  assert.throws(() => inspectPaths(Array.from({ length: 1000 }, (_, n) => Array.from({ length: 31 }, (_, d) => `${n}-${d}`).join('/') + '/f')) , /entries/);
});

test('plan is independent of input ordering and safe when inspected again', () => {
  const paths = ['A/z.txt', 'a/y.txt', 'NUL.x', '_NUL.x', 'a?/a*.txt', 'a*/a?.txt', 'Straße', 'STRASSE', '🙂'.repeat(80) + '.png'];
  const mapping = (items) => Object.fromEntries(inspectPaths(items).entries.map((entry) => [entry.original, entry.target]));
...
test('many sibling collisions have distinct, bounded, stable targets', () => {
  const paths = Array.from({ length: 1500 }, (_, n) => [...'abcdefghijk'].map((letter, bit) => n & (1 << bit) ? letter.toUpperCase() : letter).join('') + '.txt');
  const report = inspectPaths(paths);
  assert.equal(new Set(report.entries.map((entry) => portableKey(entry.target))).size, 1500);
